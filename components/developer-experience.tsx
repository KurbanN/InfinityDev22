...
const featureLines = about.bullets
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          <div>
            <div className="flex items-center gap-2">
              ...
              <p className="font-mono text-sm font-medium uppercase tracking-wider text-accent">{about.eyebrow}</p>
            </div>
            <h2 className="mt-2 font-mono text-3xl font-bold tracking-tight sm:text-4xl text-balance">{about.title}</h2>
                ...
                <code className="text-muted-foreground">
                  {displayedText.split("\n").map((line, i) => (
                    <span key={i} className="block">
                      ...
                      ) : line.includes("Готовы к запуску") ? (
                        <span className="text-accent">{line}</span>
                      ) : (
                        <span>{line}</span>
                      )}

...
import { contacts, docsSection } from "@/content/site"
...
export function DocsSection() {
  ...
  return (
    <section id="docs" ref={sectionRef} className="relative overflow-hidden py-24 sm:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl lg:text-center">
          <div className="flex items-center justify-center gap-2">
            <Blocks className="h-4 w-4 shrink-0 text-accent" />
            <p className="text-sm font-medium uppercase tracking-wider text-accent">{docsSection.eyebrow}</p>
          </div>
          <h2 className="mt-2 font-mono text-3xl font-bold tracking-tight text-balance text-foreground sm:text-4xl">
            {docsSection.title}
          </h2>
          <p className="mt-4 text-lg text-muted-foreground">{docsSection.subtitle}</p>
        </div>
      </div>

      <div className="relative z-10 mx-auto mt-16 max-w-[1400px] px-6 lg:px-12">
        <div className="mb-16 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
          {integrations.map((integration, index) => (
            ...
            >
              ...
              <div
                className={`mb-6 flex h-10 w-10 items-center justify-center transition-colors ${
        ...
        <div
          className={`flex flex-wrap items-center justify-between gap-8 border-t border-foreground/10 pb-32 pt-12 transition-all delay-500 duration-1000 lg:pb-40 ${
            isVisible ? "opacity-100" : "opacity-0"
          }`}
        >
          <div className="flex flex-wrap gap-12">
            {docsSection.stats.map((stat) => (
